...
focused_workspace1=$(i3-msg -t get_workspaces | jq -r '.[] | select(.focused == true and .output == "'"HDMI-0"'") | .name')
...
readarray -t workspaces1 <<< $(i3-msg -t get_workspaces | jq -r '.[] | select(.output == "'"HDMI-0"'") | .name')
...
for workspace in "${workspaces2[@]}"; do
  i3-msg "workspace \"$workspace\"; move workspace to output HDMI-0"
done
